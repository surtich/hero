...
hero.init = function (iodocsApisConfig, basedir, f_callback){
	new HeroIODocs(basedir, iodocsApisConfig);
	...
	for (var apiName in iodocsApisConfig) {
		var api = require(path.join(basedir, "src/iodocs/public/data", apiName + ".json"))
		for (var i = 0; i < api.endpoints.length; i++) {
			...
			for (var j = 0; j < endpoint.methods.length; j++) {
    			...
    			for (k = 0; k < method.parameters.length; k++) {
    				...
    				hero.urls[method.HTTPMethod][method.URI][param.Name] = {location: param.Location, required: param.Required === "Y" || param.Required === true};
	...
	var iodocs 	= require('iodocs').iodocs(path.join(basedir, "src/iodocs"));
