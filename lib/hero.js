...
function createApiComments (basedir, iodocsApisConfig) {
 new HeroIODocs(basedir, config.iodocs.dir, iodocsApisConfig); 
}

function createRoutes(basedir, iodocsApisConfig) {
...
function initIODocs(basedir) {
 require('iodocs').iodocs(path.join(basedir, config.iodocs.dir));
}
...
hero.init = function (iodocsApisConfig, basedir, f_callback){
 if (!config.iodocs) {
  hero.error('param "config.iodocs" not found');
 } else {
  if (config.iodocs.filter) {
   createApiComments (basedir, iodocsApisConfig);
   initIODocs(basedir);
  }
  createRoutes(basedir, iodocsApisConfig); 
  f_callback();
 }
};
