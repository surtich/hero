...
function HeroIODocs(basedir, iodocsSubdir, apisConfig) {
 ...
 function createIrisResources(irisPaths) {
  for (var apiName in self.apis) {
   ...
   var initFile = path.join(self.basedir, "src", apiName, irisPaths.baseURI, irisPaths.initPath);    
   var initContent =  "/**___ DO NOT MODIFY. AUTOMATICALLY GENERATED BY HERO-I/O DOCS.*/\n";
   initContent += "\nif (!iris.path) {";
   initContent += "\n\tiris.path = {};";
   initContent += "\n}\n";
   initContent += "\nif (!iris.path.resource) {";
   initContent += "\n\tiris.path.resource = {};";
   initContent += "\n}\n";
   for (var i = 0; i < apiDefinition.endpoints.length; i++) {
    ...
    contents += "\n\n\t},";
    contents += "\niris.path.resource." + "_" + endpoint.name.substr(0, 1).toLowerCase() + endpoint.name.substr(1) + ");";
   createFile(file, true, contents);
   initContent += "\niris.path.resource._" + endpoint.name.toLowerCase() + " = \"" + path.join(irisPaths.resourcePaths, "_" + endpoint.name.toLowerCase() + ".js\";"); 
   }
  initContent += "\n\n/**END OF AUTOMATICALLY GENERATED BY HERO-I/O DOCS.___*/";
  createFile(initFile, "replace_end", initContent);
  }
 }
 ...
 function createFile(path, overwrite, content) {
  ...
  if (e === ERROR.empty) {
   ...
   if (!fs.existsSync(path) || overwrite === true) {
    ...
    return ERROR.noError;
   } else if (overwrite === "replace" || overwrite === "replace_end") {
